...
@Autowired
    public ArticleServiceImpl(UserService userService,Gson gson,ArticleMapper articleMapper) {
        this.gson = gson;
    ...
    public PageVO selectArticleList(ArticleListRequest articleListRequest) {
        ...
        //将标签信息转换为List集合
        List<ArticleListVO> articleListVOList = articleVOList.stream().map(articleVO -> {
        ...
        if (articleListRequest.getTags() != null && !articleListRequest.getTags().isEmpty()){
            articleListVOList.stream().filter(articleListVO -> {
